...
def main(configs, tokenizer):
    torch.manual_seed(TORCH_SEED)
    torch.cuda.manual_seed_all(TORCH_SEED)
    torch.backends.cudnn.deterministic = True

    data_path = 'data/RECCON/dm.pt'
    total_data = torch.load(data_path)
    train_loader = build_dataset(configs, total_data['train'], mode='train')
    valid_loader=build_dataset(configs,total_data["val"],mode="valid")
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=configs.tuning_bert_rate)
    ...
    for epoch in range(1, configs.epochs + 1):
        for train_step, batch in enumerate(train_loader, 1):
            ...
            _, clause_list, pairs, \
            feq, feq_mask, feq_seg, feq_len, fe_clause_len, fe_doc_len, fe_adj, feq_an, fe_an_mask, \
            fcq, fcq_mask, fcq_seg, fcq_len, fc_clause_len, fc_doc_len, fc_adj, fcq_an, fc_an_mask, _  = batch

            f_cau_pred, classi,mu, logvar = model(fcq, fcq_mask, fcq_seg, fcq_len, fc_clause_len, fc_doc_len, fc_adj,fcq_an, 'f_cau')

            loss_ec = model.loss_pre(f_cau_pred, fcq_an, fc_an_mask)
            bce_loss=model.loss_pre(classi, fcq_an,fc_an_mask)

            kl_div = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())

            alpha_kl=0.0001
            losses = (loss_ec+ bce_loss + alpha_kl*kl_div) / configs.gradient_accumulation_steps  #
            losses.backward()
            ...
            if train_step % 200 == 0:
                print('epoch: {}, step: {}, loss: {}, {}, {},{}'.format(epoch, train_step,  loss_ec,bce_loss, kl_div,losses))

        with torch.no_grad():
            pos_precision, pos_recall, pos_f1,neg_precision, neg_recall, neg_f1,macro_f1 = evaluate(configs, valid_loader, model, tokenizer)
            print(pos_precision, pos_recall, pos_f1, neg_precision, neg_recall, neg_f1, macro_f1)
            if max_result is None or macro_f1>max_result:
                early_stop_flag = 1
                max_result=macro_f1

                state_dict = {'model': model.state_dict(), 'result': max_result}
                torch.save(state_dict, 'model_dia/model_valid.pth')
...
if __name__ == '__main__':
    ...
    pos_precision, pos_recall, pos_f1,neg_precision, neg_recall, neg_f1,macro_f1 = main(configs, t)
